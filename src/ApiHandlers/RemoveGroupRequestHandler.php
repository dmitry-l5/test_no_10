...
class RemoveGroupRequestHandler extends RequestHandler{
    ...
    public function run(object $input = null){
        ...
        {
            return (object)['status'=>'success', 'message'=>"Пользователь '{$user->name}' не состоит в группе '{$group->title}'"];
        }
        GroupAssign::where([
            'user_id' => $user->id,
            'group_id'=>$group->id
        ])->delete();
        return (object)['status'=>'success', 'message'=>"Пользователь '{$user->name}' удалён из группы группу '{$group->title}'"];
    }
}